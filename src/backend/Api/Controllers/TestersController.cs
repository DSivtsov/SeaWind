...
public sealed class TestersController : ControllerBase
{
    ...
    {
        if (minAge is < 0) return BadRequest("minAge must be >= 0");
    ...
    public ActionResult<TesterDto> Create([FromBody] CreateTesterRequest body)
    {
        ...
        if (body.Age < 0 || body.Age > 150) return BadRequest("Age must be between 0 and 150");

        var result = _svc.Create(body);
        if (result.IsConflict) return Conflict(result.Error);
        if (result.IsBadReq) return BadRequest(result.Error);
