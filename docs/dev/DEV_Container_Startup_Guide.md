...
## 2. Docker — сборка и запуск контейнеров
(требуется отдельная консоль для запуска контейнеров в режиме консольного мониторинга)
...
❗ Рекомендуется в контейнер запускать только базу данных
```bash
# Запуск только базы данных
docker compose up db                # Просмотрите вывод, чтобы не было ошибок.
```

⚠️ Запуск API в контейнере — рекомендуется только если это действительно необходимо и код backend готов к работе
```bash
#Запуск базы данных вместе с баскендом 
docker compose up                   # Просмотрите вывод, чтобы не было ошибок.
```
...
После успешного применения миграций можно использовать БД для разработки (или тестирования) вместе с backend и фронтендом (для фронтенда ничего не поменялось).
- В src\backend\Api\Properties\launchSettings.json настроены несколько профилей для работы в DEV.
- Основной - "Api", при его использовании - у вас "код будет работать одинаково" как в VS и dotnet, так и в контейнере.
- При запуске в VS - вы можете выбирать Debug/Release, но при запуске через dotnet, контейнер всегда будет Release, также как и в CI.
### 🔹 Запуск backend через dotnet
```bash
# для запуска нужна отдельная консоль
dotnet run --project src/backend/Api    # по умолчанию запуститься профайл "Api"
```
или
```bash
# через Visual Studio:
# 1. Выполни Clean/Rebuild Solution.  
# 2. Запусти профайл "Api" — автоматически откроется **Swagger**.
```
...
## Change Log 
- v2 (2025-10-13) — форматирование и уточнение формулировок, добавлено "Запуск API в контейнере"
